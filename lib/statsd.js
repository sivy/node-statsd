 ...
 * @const
 */
var MAX_BATCH_DELAY = 1000;

/**
 * @const
 */
var MAX_BATCH_MESSAGES = 20;
...
Client.prototype._addToBatch = function(message, callback) {
  ...
  if (this.pendingMessages.length > MAX_BATCH_MESSAGES) {
    this._sendBatch();
    ...
    // after MAX_BATCH_DELAY
    this.sendTimeout = setTimeout(this._sendBatch.bind(this), MAX_BATCH_DELAY);
  }
};
...
Client.prototype.close = function(){
    if (this.batch && this.sendTimeout) {
      clearTimeout(this.sendTimeout);
      this.sendTimeout = null;
    }
    this.socket.close();
